...
base_path = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # 取得目前檔案的路徑
...
model_path = os.path.join(base_path,'output', 'model')  # 設定模型路徑
...
class SharedLinearModel(nn.Module):
    def __init__(self, input_dim):
        super(SharedLinearModel, self).__init__()
        self.models = nn.ModuleList([nn.Linear(input_dim, 1)])
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        # outputs = [self.models[i](x[:, i, :]).squeeze() for i in range(len(self.models))]
        # outputs = torch.stack(outputs, dim=1)
        outputs = self.models[0](x).squeeze()
...
num_tickers = n
input_dim = o
model = SharedLinearModel(input_dim).to(device)
...
if 1: # 取得模型權重
    new_model = SharedLinearModel(input_dim).to(device)
